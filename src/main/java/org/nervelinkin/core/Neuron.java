 ...
 * 2、对于输入信号量的加工由神经元本身特性决定，包括三种处理方式：化合、选择、比较
 * 3、对于输出信号量输出，是一个固定值，当神经元连接简历，输出信号量的类型就固定了。一个神经元可以决定输入信号的编码
 * 4、输入信号量强度的强度，是由神经元激活期内的输入信号持续时间来决定
 ...
 * 9、神经元可以接受多种输入信号量，也可以输出多种信号量
 * 10、神经元根据功能需要，会主动产生输入信号的记忆，这种记忆是永久的
 * 11、神经元间如果有信号传输，神经连接的权重自动增加。如果权重为零，神经元此类的记忆便不可被检索。
 * 12、带记忆的神经元可以接受查询信号，激活记忆，传递计算输出
 */
